...
const certificates = [
  {
    ...
    imageUrl: "/ibm_ai_fundamentals.png",
  ...
  },
  {
    ...
    imageUrl: "/ibm_ai_customer_service.png",
...
const CertificatesSection = () => {
  ...
  // Slide animation variants tailored to stay within screen boundaries
  const slideVariants = {
    enter: (dir) => ({
      x: dir > 0 ? 100 : -100,
      opacity: 0
  ...
  // Static preview card for SSR to avoid hydration mismatches
  if (!isMounted) {
    return (
        ...
        <div className="relative max-w-6xl mx-auto px-6 lg:px-8">
          ...
          <div className="relative w-full">
            <div className="w-full overflow-hidden px-1 py-1">
              <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-10 items-center bg-card/40 border border-border rounded-2xl p-6 sm:p-10 shadow-brand">
                <div className="lg:col-span-8 w-full h-[240px] sm:h-[400px] lg:h-[480px] rounded-xl overflow-hidden bg-white border border-border shadow-brand-lg relative flex items-center justify-center p-3 sm:p-5">
                  <img
                ...
                <div className="lg:col-span-4 space-y-6 text-left flex flex-col justify-center">
                  <div>
                    ...
                    <div className="mt-3 flex items-center space-x-2 text-sm text-muted-foreground font-semibold">
                      ...
                      <span className="text-foreground">{certificates[0].issuer}</span>
    ...
    <section id="certificates" className="relative overflow-hidden scroll-mt-20 py-20 bg-transparent">
      <div className="relative max-w-6xl mx-auto px-6 lg:px-8">
        ...
        <div className="relative w-full">
          ...
          {/* Inner Wrapper to contain and clip the slide translations inside the webpage */}
          <div className="w-full overflow-hidden px-1 py-1">
            <AnimatePresence initial={false} custom={direction} mode="wait">
              <motion.div
                ...
                animate="center"
                exit="exit"
                className="w-full grid grid-cols-1 lg:grid-cols-12 gap-10 items-center bg-card/40 border border-border rounded-2xl p-6 sm:p-10 shadow-brand"
              >
                
                {/* Left Column: High-Quality Certificate Image Preview */}
                <div className="lg:col-span-8 w-full h-[240px] sm:h-[400px] lg:h-[480px] rounded-xl overflow-hidden bg-white border border-border shadow-brand-lg relative group select-none flex items-center justify-center p-3 sm:p-5">
                  <img
                  ...
                  {/* Hover Maximize Overlay (links directly to authentic PDF) */}
          ...
          {/* Left Arrow Button - Positioned responsively inside or outside based on screen size */}
          <button
            onClick={prevSlide}
            className="absolute left-2 lg:left-[-16px] xl:left-[-24px] top-1/2 transform -translate-y-1/2 z-30 w-12 h-12 rounded-full border border-border bg-card/90 text-foreground hover:text-primary hover:border-primary/40 flex items-center justify-center transition-all duration-300 hover:scale-105 active:scale-95 shadow-brand-lg focus:outline-none hidden md:flex"
            aria-label="Previous Certificate"
          >
            <Icon name="ChevronLeft" size={24} />
          </button>

          {/* Right Arrow Button - Positioned responsively inside or outside based on screen size */}
          <button
            onClick={nextSlide}
            className="absolute right-2 lg:right-[-16px] xl:right-[-24px] top-1/2 transform -translate-y-1/2 z-30 w-12 h-12 rounded-full border border-border bg-card/90 text-foreground hover:text-primary hover:border-primary/40 flex items-center justify-center transition-all duration-300 hover:scale-105 active:scale-95 shadow-brand-lg focus:outline-none hidden md:flex"
        ...
        <div className="flex md:hidden justify-center gap-4 mt-6 select-none">
          <button
            onClick={prevSlide}
